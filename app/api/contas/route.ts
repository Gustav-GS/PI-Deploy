...
export async function GET() {
  await ensureInit();
  const { rows } = await sql`SELECT id, created_at, title, content, image_urls FROM prestacaocontas ORDER BY created_at DESC`;
...
export async function POST(req: Request) {
  ...
  const { rows } = await sql<{ id: number }>`
    INSERT INTO prestacaocontas (title, content, image_urls) VALUES (${title}, ${content}, ${JSON.stringify(urls)}::jsonb) RETURNING id
  `;
